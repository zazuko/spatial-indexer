...
import { GRAPH_URI, SRS_URI, features, toTurtle } from './fixtures/cities.js'
import { isAvailable, runOrFail } from './helpers/exec.js'
import { fusekiLogs, pullFusekiImage, startFuseki, stopFuseki } from './helpers/fuseki.js'
import { ensureJar, parseIndexHeader, runIndexer } from './helpers/indexer.js'
import { ensureJenaTools, loadDataset } from './helpers/jena.js'
import { TMP_DIR } from './helpers/paths.js'
import { select, selectColumn } from './helpers/sparql.js'
 ...
 *
 * @returns {{ baseUrl: string, endpoint: string, name: string }}
 */
function requireFuseki() {
...
async function assertPrerequisites() {
  ...
  for (const command of required) {
    if (!(await isAvailable(command))) missing.push(command)
  }
  if (missing.length > 0) {
    throw new Error(
...
describe('spatial index generation', { timeout: SETUP_TIMEOUT }, () => {
  it('exits successfully', () => {
    assert.equal(
      context.indexerResult.code,
      0,
      `indexer failed:\n${context.indexerResult.stdout}\n${context.indexerResult.stderr}`,
    )
  })

  it('reports no error', () => {
    const output = `${context.indexerResult.stdout}\n${context.indexerResult.stderr}`
  ...
  it('detects the SRS on its own when --srs is omitted', async () => {
    assert.equal(
      context.autoSrsResult.code,
      0,
      `indexer failed:\n${context.autoSrsResult.stdout}\n${context.autoSrsResult.stderr}`,
    )
...
describe('GeoSPARQL queries', { timeout: SETUP_TIMEOUT }, () => {
  ...
  it('finds nearby features through the spatial index', async () => {
    ...
    assert.deepEqual(await near(300), ['Basel', 'Bern', 'Geneva', 'Lake Zurich', 'Lugano', 'Zurich'])
  })
