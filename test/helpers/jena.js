...
import { CACHE_DIR, PROJECT_ROOT } from './paths.js'
 ...
 */
export async function jenaVersion() {
  const pom = await readFile(join(PROJECT_ROOT, 'pom.xml'), 'utf8')
  const match = pom.match(/<jena\.version>([^<]+)<\/jena\.version>/)
  if (!match) throw new Error('Could not read <jena.version> from pom.xml')
  return match[1]
}
 ...
 *
 * @returns {Promise<string>} path to the Jena distribution directory
 */
export async function ensureJenaTools() {
 ...
 *
 * @param {{ jenaHome: string, datasetPath: string, turtlePath: string, graph: string }} options
 * @returns {Promise<void>}
 */
export async function loadDataset({ jenaHome, datasetPath, turtlePath, graph }) {
